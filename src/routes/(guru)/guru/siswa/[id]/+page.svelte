...
<svelte:head>
	<title>Detail Pembimbing Siswa — {student.fullName || 'Siswa'} | Portal Guru NLC</title>
</svelte:head>
...
<div class="page-container">
	...
	<PageHeaderCard
		...
		breadcrumbs={[
			{ label: 'Health Monitoring', href: `/guru/monitoring?kelasId=${student.kelasId || ''}` },
			{ label: 'Detail Siswa' }
		]}
	>
		{#snippet badges()}
			<a href="/guru/monitoring?kelasId={student.kelasId || ''}" class="btn-secondary-head-pill">
				...
				<div>
					<div class="flex items-center gap-2 flex-wrap">
						...
						{#if summary.riskLevel === 'KRITIS'}
							<span class="badge badge-revisi">RISK KRITIS</span>
						{:else if summary.riskLevel === 'WASPADA'}
							<span class="badge badge-pending">RISK WASPADA</span>
						{:else}
							<span class="badge badge-approved">RISK SEHAT</span>
						{/if}
					</div>
					<p class="student-hero-sub">
						@{student.username || ''} · {student.tahunAjaranName || ''}
					</p>
				</div>
			</div>
	...
	{#if summary.alertReasons && summary.alertReasons.length > 0}
		<div class="alert-warning-box">
			<div class="alert-warning-header">
				...
				<span class="alert-warning-title">Peringatan Pendampingan Siswa Required</span>
			</div>
			...
			<div class="tab-section">
				<div class="section-header">
					<div>
						...
						<p class="section-sub">Tuliskan catatan umpan balik, pendampingan, atau arahan bimbingan untuk {student.fullName}.</p>
					</div>
				</div>

				<!-- Add Note Form for Guru -->
				<div class="add-note-box bg-white border border-slate-200 rounded-xl p-4 mb-4 shadow-xs">
					<h3 class="font-bold text-slate-900 text-sm mb-1">Tambah Catatan Pendampingan Baru</h3>
					{#if form?.error}
						<div class="p-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg text-xs font-semibold mb-3">
							{form.error}
						</div>
					{/if}
					{#if form?.success}
						<div class="p-3 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg text-xs font-semibold mb-3">
							{form.message}
						</div>
					{/if}
					...
					>
						...
						<div class="flex justify-end">
							<button
								...
								class="btn-create-pill border-none cursor-pointer text-xs"
							>
