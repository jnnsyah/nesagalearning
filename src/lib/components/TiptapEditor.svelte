<script lang="ts">
	...
	// ── Floating text bubble menu (appears on text selection) ──
	let bubbleVisible = $state(false);
	let bubbleX       = $state(0);
	let bubbleY       = $state(0);
	// mirror active states for bubble
	...
	let bubbleBullet   = $state(false);
	let bubbleOrdered  = $state(false);
	let bubbleQuote    = $state(false);
	...
	function updateToolbarState() {
		...
		bubbleH2      = isHeading2;
		bubbleH3      = isHeading3;
		bubbleBullet  = isBulletList;
		bubbleOrdered = isOrderedList;
		bubbleQuote   = isBlockquote;
	...
	// Update bubble menu position based on browser selection
	function updateBubbleMenu() {
		...
		// Center bubble above selection
		bubbleX = selRect.left - rootRect.left + selRect.width / 2;
		bubbleY = selRect.top - rootRect.top - 8; // 8px gap above selection
		bubbleVisible = true;
	}

	// Update floating image toolbar position
	function updateImageToolbar() {
		if (!editor || !editorRoot) return;
		if (!isImage) { imgToolbarVisible = false; return; }

		// Find the selected image wrapper
		...
		imgToolbarY = rect.top - rootRect.top - 8;
	...
	onMount(() => {
		...
		editor = new Editor({
			...
			onBlur: () => {
				// Delay so bubble menu clicks can register
				setTimeout(() => {
					bubbleVisible = false;
		...
		const onDocClick = (e: MouseEvent) => {
			...
			if (!t.closest('.tiptap-image-toolbar') && !t.closest('.tiptap-image-wrapper')) {
				imgToolbarVisible = false;
			}
			if (!t.closest('.tiptap-bubble-menu') && !t.closest('.ProseMirror')) {
				bubbleVisible = false;
			}
		};
...
{#if showImageDialog}
	...
	<div class="img-dialog-backdrop" onclick={closeImageDialog} role="dialog" aria-modal="true">
		<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
...
<div class="editor-container" class:editor-container--disabled={disabled} bind:this={editorRoot}>
	...
	<!-- === CANVAS WRAP (floating toolbars live here) === -->
	<div class="editor-canvas-wrap">
		...
		{#if imgToolbarVisible && isImage}
			...
			>
				<button type="button" class="ftb-btn" class:ftb-btn--active={activeImageAlign === 'left'}
				...
				<button type="button" class="ftb-btn" class:ftb-btn--active={activeImageAlign === 'center'}
				...
				<button type="button" class="ftb-btn" class:ftb-btn--active={activeImageAlign === 'right'}
				...
				<button type="button" class="ftb-btn" class:ftb-btn--active={activeImageAlign === 'full'}
				...
				<button type="button" class="ftb-btn ftb-btn--danger"
					onmousedown={(e) => { e.preventDefault(); deleteImage(); }} title="Hapus">
		...
		{#if bubbleVisible}
			...
			>
				<div class="bubble-group">
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleBold}
					...
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleItalic}
					...
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleCode}
				...
				<div class="ftb-divider"></div>
				<div class="bubble-group">
					<button type="button" class="ftb-btn ftb-btn--text" class:ftb-btn--active={bubbleH1}
						onmousedown={(e) => { e.preventDefault(); editor?.chain().focus().toggleHeading({ level: 1 }).run(); }}>H1</button>
					<button type="button" class="ftb-btn ftb-btn--text" class:ftb-btn--active={bubbleH2}
						onmousedown={(e) => { e.preventDefault(); editor?.chain().focus().toggleHeading({ level: 2 }).run(); }}>H2</button>
					<button type="button" class="ftb-btn ftb-btn--text" class:ftb-btn--active={bubbleH3}
						onmousedown={(e) => { e.preventDefault(); editor?.chain().focus().toggleHeading({ level: 3 }).run(); }}>H3</button>
				</div>
				<div class="ftb-divider"></div>
				<div class="bubble-group">
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleBullet}
					...
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleOrdered}
					...
					<button type="button" class="ftb-btn" class:ftb-btn--active={bubbleQuote}
...
<style>
	...
	.editor-container {
		...
		/* overflow:hidden keeps border-radius clip AND allows internal scroll */
		overflow: hidden;
	...
	/* ===== CANVAS WRAP: overflow visible so floating toolbars can escape ===== */
	.editor-canvas-wrap {
		position: relative;
		/* Allow floats to overflow the wrap but clip is reset by the outer container */
		overflow: visible;
		flex: 1;
	}

	/* ===== FLOATING TOOLBARS (shared) ===== */
	.tiptap-image-toolbar,
	.tiptap-bubble-menu {
		position: absolute;
		/* Center horizontally on X, sit above Y */
		...
		border-radius: 10px;
		padding: 5px 7px;
		box-shadow: 0 8px 24px rgba(15, 23, 42, 0.35), 0 2px 8px rgba(15, 23, 42, 0.2);
		pointer-events: auto;
		white-space: nowrap;
	...
	/* Shared floating toolbar button */
	.ftb-btn {
		display: flex;
		align-items: center;
	...
	.ftb-btn:hover { background: #1e293b; color: #f1f5f9; }
	.ftb-btn--active { background: #4f46e5 !important; color: white !important; border-color: #6366f1 !important; }
	.ftb-btn--text { font-family: var(--font-macro); font-size: 11px; font-weight: 800; }
	.ftb-btn--danger:hover { background: #450a0a !important; color: #fca5a5 !important; }
	...
	.tool-btn:hover:not(:disabled) { background: #ffffff; border-color: var(--border-hard); color: var(--text-primary); }
	.tool-btn--active { background: var(--primary-light) !important; border-color: var(--primary-border) !important; color: var(--primary) !important; }
	.tool-btn--image:hover:not(:disabled) { color: #059669; background: var(--green-dim) !important; border-color: var(--green-border) !important; }
	.tool-btn:disabled { opacity: 0.35; cursor: not-allowed; }
	.text-btn { font-family: var(--font-macro); font-size: 12px; font-weight: 800; }
	...
	:global(.editor-content .ProseMirror) {
		...
		line-height: 1.7;
		/* Allow editor itself to scroll if content is long */
		overflow-y: auto;
	}
	...
	.img-dialog__title {
		...
		font-family: var(--font-macro); font-size: 15px; font-weight: 800; color: var(--text-primary);
	}
	.img-dialog__close {
		display: flex; align-items: center; justify-content: center;
		width: 30px; height: 30px; border-radius: var(--radius-md);
		border: 1px solid var(--border-hard); background: white; color: var(--text-muted); cursor: pointer;
		transition: all 150ms ease;
	}
	...
	.img-field__input {
		...
		color: var(--text-primary); outline: none; transition: border-color 150ms, box-shadow 150ms;
	}
	...
	.img-divider-text { font-family: var(--font-mono); font-size: 10px; color: var(--text-muted); font-weight: 600; white-space: nowrap; }
	...
	.img-upload-label {
		...
		border-radius: var(--radius-md); font-family: var(--font-body); font-size: 13px;
		font-weight: 600; color: var(--text-secondary); cursor: pointer; transition: all 150ms;
	...
	.img-btn-insert {
		...
		cursor: pointer; box-shadow: 0 4px 12px -2px rgba(79,70,229,0.3); transition: all 150ms;
	}
